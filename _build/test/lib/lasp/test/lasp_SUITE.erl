...
%% @doc Test transaction functionality.
transaction_test(_Config) ->
    ...
    %% Apply .transaction
    ?assertMatch(ok,lasp:transaction([{SourceId, {add, 2}},{SourceId, {add, 1}},{DestinationId, {add, 3}}], self())),
    
     {ok, Result} = lasp:query(SourceId), sets:to_list(Result),
     {ok, Result1} = lasp:query(DestinationId), sets:to_list(Result1),
    
     %%% resultat
	?assertEqual({ok, sets:from_list([3])}, {ok, Result1}),
    ?assertEqual({ok, sets:from_list([2,1])}, {ok, Result}),
